...
namespace Spartacus.UnitTests
{
    public class LinearTermsTests
    {
        ...
        public void Calculate_MoreVariables_ReturnCorrectResult()
        {
            ...
            Assert.Equal(1, examples[0].Variables.Count(c => c.Schema.VariableType == VariableType.Linear && c.Value == 5));
            Assert.Equal(1, examples[0].Variables.Count(c => c.Schema.VariableType == VariableType.Linear && c.Value == -1));
            Assert.Equal(1, examples[0].Variables.Count(c => c.Schema.VariableType == VariableType.Linear && c.Value == 1));

            Assert.Equal(1, examples[0].Variables.Count(c => c.Schema.VariableType == VariableType.Linear && c.Schema.Symbol.Equals("X+Y")));
            Assert.Equal(1, examples[0].Variables.Count(c => c.Schema.VariableType == VariableType.Linear && c.Schema.Symbol.Equals("X-Y")));
            Assert.Equal(1, examples[0].Variables.Count(c => c.Schema.VariableType == VariableType.Linear && c.Schema.Symbol.Equals("Y-X")));
